...
class Joint_bilateral_filter(object):
    ...
    def joint_bilateral_filter(self, img, guidance):
        ...
        gaussian_kernel = np.exp(-1*((np.power(grid_x, 2) + np.power(grid_y, 2)) / (2*self.sigma_s**2))).flatten()
        ...
        unique_value = np.arange(256)
        unique_value_normalized = unique_value / 255.
        new_unique_value = np.zeros(256, dtype=np.float64)
        new_unique_value[unique_value] = unique_value_normalized
        unique_value = new_unique_value
        print(unique_value.dtype)
        lookup_table_mat = unique_value[:, None]-unique_value
        
        lookup_table_mat = np.exp(-np.power(lookup_table_mat, 2)/(2*(self.sigma_r**2)))
        ...
        np.save('kernal_sum', sum_of_combined_kernel)
